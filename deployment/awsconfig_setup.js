    ...
    async.doWhilst(
        ...
        function () {
            if (setupData.configurationRecorders.length) {
                ...
                setupData.logger("No configuration recorders setup for '" + setupData.region + "'.");
        ...
        function (err) {
            if (err) {
                setupData.logger("describeConfigurationRecorders failed for region '" + setupData.region + "'. Error: " + err);
                resultCallback(err);
            } else {
                AWS.config.update({region: setupData.region});
                ...
                awsConfig.describeDeliveryChannels(function (err, data) {
                    if (err) {
                        setupData.logger("describeDeliveryChannels failed for region '" + setupData.region + "'. Error: " + err);
                        resultCallback(err); 
                    } else {
                        setupData.deliveryChannels = data.DeliveryChannels;
                        setupData.logger("Found valid AWS Config setup -" 
...
function setupAwsConfigRecorder(setupData, resultCallback) {
    ...
    async.waterfall([
        ...
        function(err, result) {
            if (err) {
                setupData.logger("Failed to configure AWS Config for '" + setupData.setupRegion + "'. Error: " + JSON.stringify(err));
                return resultCallback(err);
            } else {
                setupData.logger("Successfully configured AWS Config for '" + setupData.setupRegion + "'.");
        ...
        if (err) {
            switch (err.code) {
                case "ThrottlingException":
                    setupData.logger("Delivery of a configuration snapshot was already scheduled.");
                    return resultCallback(null, setupData);
                default:
                    setupData.logger("Failed to schedule delivery of a configuration snapshot. Error: " + JSON.stringify(err));
                    return resultCallback(err);
            }
        } else {
            setupData.logger("Successfully scheduled delivery of a configuration snapshot. configSnapshotId: " + data.configSnapshotId);
            return resultCallback(null, setupData);
        }
    });
}
